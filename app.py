...
# --- Model Loading with Caching and Download Logic ---

# Custom object for MobileNetV2 models
...
def download_model(filename, url):
    """Downloads a single model file if it doesn't already exist."""
...
def load_all_models():
    """
    Ensures all models are downloaded and then loads them into memory.
    This function is cached, so it only runs once.
    """
    models = {}
    
    # Dictionary of model names, their remote URLs, and local paths
    model_info = {
        "acne": {
            "path": "acne1_detector_mobilenetv2.h5",
            "url": "https://huggingface.co/vaishnavisinha11/model/blob/main/acne1_detector_mobilenetv2.h5"
        },
        "dark": {
            "path": "dark_detector.h5",
            "url": "https://huggingface.co/vaishnavisinha11/model/blob/main/dark_detector.h5"
        },
        "pigment": {
            "path": "pigmentation_detector.h5",
            "url": "https://huggingface.co/vaishnavisinha11/model/blob/main/pigmentation_detector.h5"
        },
        "skin": {
            "path": "final_skin_type_model_resnet.h5",
            "url": "https://huggingface.co/vaishnavisinha11/model/blob/main/final_skin_type_model_resnet.h5"
        }
    }

    # First, download all models that are missing
    st.info("Checking for model files...")
    for name, info in model_info.items():
        download_model(info['path'], info['url'])
    ...
    for name, info in model_info.items():
        ...
        if os.path.exists(path):
            try:
                # Use custom objects only for the model that needs it
                if name == "acne":
            ...
            st.warning(f"Could not find or download model file: {path}")
            models[name] = None
